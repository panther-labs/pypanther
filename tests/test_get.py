...
from pypanther.get import get_panther_rules, get_rules, run_rules
...
class TestRunRules:
    def test_not_found(self) -> None:
        ...
        rc, err_msg = run_rules(argparse.Namespace(id=rule_id, type=TYPE_RULE.lower(), output="text"))
        assert rc == 1
    ...
    def test_invalid_output(self) -> None:
        ...
        rc, err_msg = run_rules(argparse.Namespace(id=rule_id, type=TYPE_RULE.lower(), output=fake_output))
        assert rc == 1
    ...
    def test_happy_path_text(self, rule: Type[Rule]) -> None:
        rc, err_msg = run_rules(argparse.Namespace(id=rule.id, type=TYPE_RULE.lower(), output="text"))
        assert rc == 0
        assert err_msg == ""

    @pytest.mark.parametrize("rule", get_panther_rules(), ids=lambda x: x.id)
    def test_happy_path_json(self, rule: Type[Rule]) -> None:
        rc, err_msg = run_rules(argparse.Namespace(id=rule.id, type=TYPE_RULE.lower(), output="json"))
        assert rc == 0
        assert err_msg == ""
