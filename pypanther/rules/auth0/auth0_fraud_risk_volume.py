...
@panther_managed
class Auth0FraudRiskVolume(Rule):
    id = "Auth0.FraudRisk.Volume-prototype"
    log_types = [LogType.AUTH0_EVENTS]
    display_name = "Auth0 Fraud Risk by Volume"
    default_description = "Detects a surge in either failed, successful or suspicious login attempts using leaked passwords over a window of time and a threshold. Exceeding set threshold may indicate potential fraud."
    default_severity = Severity.HIGH
    reports = {"MITRE ATT&CK": ["TA0003:T1136"]}
    default_reference = (
        "https://github.com/auth0/auth0-customer-detections/blob/main/detections/risk_of_signup_fraud_by_volume.yml"
    )
    threshold = 20
    SUSPICIOUS_EVENT_TYPES = ("fs", "ss", "signup_pwd_leak")
    ...
    def title(self, event):
        ...
        return f"Auth0 User [{user}] had a surge of suspicious [{event_type}] event in your organization's tenant [{p_source_label}]."
