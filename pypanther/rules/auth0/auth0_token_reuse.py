...
@panther_managed
class Auth0RefreshTokenReuse(Rule):
    default_description = "A refresh token was reused."
    display_name = "Auth0 Refresh Token Reused"
    ...
    id = "Auth0.RefreshToken.Reuse-prototype"
    ...
    def rule(self, event):
        ...
        return all(
            [
                data_description == "Unsuccessful Refresh Token exchange, reused refresh token detected",
                data_type == "ferrt",
    ...
    def title(self, event):
        ...
        return (
            f"Auth0 User [{user}] attempted to reuse a refresh token foryour organization's tenant [{p_source_label}]."
        )
