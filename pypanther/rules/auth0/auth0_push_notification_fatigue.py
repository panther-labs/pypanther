...
class Auth0PushNotificationFatigue(Rule):
    ...
    id = "Auth0.PushNotification.Fatigue-prototype"
    threshold = 5

    def rule(self, event):
        data_type = event.deep_get("data", "type", default="<NO_DATA_TYPE_FOUND>")
        return all([data_type == "gd_send_pn", is_auth0_config_event(event)])

    def title(self, event):
        user = event.deep_get("data", "details", "request", "auth", "user", "email", default="<NO_USER_FOUND>")
        return f"Auth0 User [{user}] has received an excessive number of MFA push notifications,possible MFA fatigue detected"
