...
@panther_managed
class AWSS3DisableMfaDelete(Rule):
    id = "AWS.S3.DisableMfaDelete-prototype"
    display_name = "S3 MFA Delete Disabled"
    log_types = [LogType.AWS_CLOUDTRAIL]
    tags = ["AWS", "Defense Evasion", "Impact:Data Destruction"]
    reports = {"MITRE ATT&CK": ["TA0005:T1562", "TA0040:T1485"]}
    default_severity = Severity.LOW
    status = "Experimental"
    default_description = "Detects when MFA Delete is disabled on an S3 bucket, removing an important security control that prevents accidental or malicious deletion of versioned objects and could indicate ransomware preparation activity."
    default_runbook = "1. Query CloudTrail for all S3 API calls by the userIdentity:arn on the requestParameters:bucketName in the 24 hours before and after MFA Delete was disabled\n2. Find any DeleteObject or DeleteObjects events on this bucket in the 6 hours after MFA Delete was disabled to check if bulk deletion occurred\n3. Search for other security control changes (DisableBucketLogging, SuspendVersioning, DeleteBucketEncryption) on the same bucket in the past 7 days to identify ransomware preparation patterns\n"
    default_reference = "https://docs.aws.amazon.com/AmazonS3/latest/userguide/MultiFactorAuthenticationDelete.html"
    ...
    def rule(self, event):
        return (
            ...
            and (event.deep_get("requestParameters", "VersioningConfiguration", "MfaDelete") == "Disabled")
        )

    def title(self, event):
        return f"[AWS.CloudTrail] User [{event.udm('actor_user')}] disabled MFA Delete feature for bucket [{event.deep_get('requestParameters', 'bucketName')}] "
        ...
        RuleTest(
            ...
            log={
                ...
                "userIdentity": {
                    ...
                    "sessionContext": {
                        "attributes": {"mfaAuthenticated": "true", "creationDate": "2024-01-15T10:30:00Z"},
                        "sessionIssuer": {
                            "type": "Role",
