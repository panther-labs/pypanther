...
cloudflare_firewall_high_volume_events_blocked_grey_noise_tests: List[PantherRuleTest] = [
    PantherRuleTest(
...
class CloudflareFirewallHighVolumeEventsBlockedGreyNoise(PantherRule):
    RuleID = "Cloudflare.Firewall.HighVolumeEventsBlockedGreyNoise-prototype"
    ...
    Tests = cloudflare_firewall_high_volume_events_blocked_grey_noise_tests
    ...
    def rule(self, event):
        ...
        if self.NOISE.classification("ClientIP") == "malicious":
            return True
        return False

    def title(self, event):
        return f"Cloudflare: High Volume of Block Actions - from [{event.get('ClientIP', '<NO_CLIENTIP>')}] to [{event.get('ClientRequestHost', '<NO_REQ_HOST>')}]  - GreyNoise identified IP as malicious"
