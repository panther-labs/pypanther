...
class OneLoginUnusualLogin(PantherRule):
    RuleID = "OneLogin.UnusualLogin-prototype"
    ...
    SummaryAttributes = ["account_id", "user_id", "user_name", "ipaddr"]
    Tests = one_login_unusual_login_tests
    ...
    def rule(self, event):
        ...
        self.EVENT_LOGIN_INFO[event.get("p_row_id")] = login_tuple
        ...
        for tcount in last_login_info.values():
            if tcount > tuple_count:
                higher_tuples += 1
            if higher_tuples >= self.FINGERPRINT_THRESHOLD:
                return True
        return False
    ...
    def title(self, event):
        ...
        return f"Unusual OneLogin access for user [{event.get('user_name', '<UNKNOWN_USER>')}] from [{self.EVENT_LOGIN_INFO[event.get('p_row_id')]}]"
