...
from panther_analysis.helpers.panther_base_helpers import deep_get, pattern_match_list
...
awss3_grey_noise_activity_tests: List[PantherRuleTest] = [
    ...
    PantherRuleTest(
        ...
        Log={
            ...
            "p_any_aws_account_ids": ["111122223333"],
            "p_any_aws_arns": ["arn:aws:iam::111122223333:role/Panther", "arn:aws:s3:::panther"],
        },
    ),
    PantherRuleTest(
    ...
    PantherRuleTest(
        ...
        Log={
            ...
            "p_rule_tags": ["AWS", "Collection:Data From Cloud Storage Object", "GreyNoise"],
...
class AWSS3GreyNoiseActivity(PantherRule):
    ...
    _ALLOWED_ROLES = ("*PantherAuditRole-*", "*PantherLogProcessingRole-*")
    ...
    def rule(self, event):
        ...
        if not pattern_match_list(event.get("eventName"), self._S3_EVENT_LIST):
            return False
        ...
        if self.NOISE.classification("sourceIPAddress") == "malicious":
            # Filter: Roles that generate FP's if used from AWS IP Space
            if pattern_match_list(deep_get(event, "userIdentity", "arn"), self._ALLOWED_ROLES):
                # Only Greynoise advanced provides AS organization info
                if self.NOISE.subscription_level() == "advanced":
                    if self.NOISE.organization("sourceIPAddress") == "Amazon.com, Inc.":
                        return False
                else:
    ...
    def alert_context(self, event):
        return self.NOISE.context("sourceIPAddress")
