...
from panther_analysis.base import PantherDataModel, PantherRule
...
def get_panther_rules(**kwargs):
    """Return an iterator of all PantherRules in the panther_analysis.rules based on the provided filters.
...
def get_panther_data_models(**kwargs):
    """Return an iterator of all PantherDataModels in the panther_analysis.rules based on the provided filters.
    ...
    if not __DATA_MODELS:
        p_a_d = import_module("panther_analysis.data_models")
        for module_info in walk_packages(p_a_d.__path__, "panther_analysis.data_models."):
            m = import_module(module_info.name)
            for item in dir(m):
                attr = getattr(m, item)
                if (
